                    ...
                    for (int i = 0; i < list.size(); i++) {
                        print(list[i])
                         if ("${list[i]}" == "'UnitTests'" && env.ACTION == 'DEPLOY') {
                            stage('Unit Tests') {
                                sh """
                                docker run --rm --user root -v "$WORKSPACE":/opt/repo -w /opt/repo $NODE_IMAGE /bin/bash -c "cd /opt/repo &&  npm install && npm run test -- --watchAll=false && npm run test:cov --watchAll=false"
